...
def tools_to_str(tools: list[Tool]) -> str:
    ...
    safe_tools = [t for t in tools if t.name not in ["run_command", "write_file", "bash"]]
    
    # Many libraries use .openai_schema as a shortcut for "Standard JSON Schema"
    # If using a library like A2A, this is the expected field for all models.
    return json.dumps([tool.openai_schema for tool in safe_tools], indent=2)
...
def get_green_agent_config(user_input: str) -> tuple[str, EnvConfig]:
    ...
    try:
        ...
        # SSRF protection: Block dangerous internal IP addresses
        from urllib.parse import urlparse
        parsed = urlparse(white_agent_url)
        if parsed.hostname in ["localhost", "127.0.0.1", "0.0.0.0"]:
            raise ValueError(f"Internal/Localhost URLs are not allowed: {white_agent_url}")
